 ...
 */
 ...
 /* Modified for rocketjumper */
 
class RJ_TilemapExt extends FlxTilemap
{
	...
	private function bounceDownSlope():Void
	{
	 ...
	 * @param	FacingEast	If the sloped tile is facing towards the east
	 */
	private function onCollideFloorSlope(Slope:FlxObject, Object:FlxObject, FacingEast:Bool):Void
	{
		// Set the object's touching flag
		Object.touching = FlxObject.FLOOR;
		
		// Do a bunch of magic to make bouncing up and down slopes feel more like an FPS game
		//@TODO: Maybe make this only while holding jump?
		...
		var movingWest:Bool = Object.velocity.x < -threshold;
		var movingEast:Bool = Object.velocity.x > threshold;
		...
		if ( movingEast )
		{
			...
			{
				// Bounce downhill (to the east)
				Object.velocity.x += Object.velocity.y * 0.5;
		...
		var targetPoint:Float = _slopePoint.y - Object.height;
		var targetInt:Int = Std.int(targetPoint);
		...
		if ( didBounce )
		{
			Object.y -= 1; // Hack to prevent this function being called multiple times on a bounce (setting velocity alone isn't enough)
		}
	}
	 ...
	 * @param	FacingEast	If the sloped tile is facing towards the east
	 */
	private function onCollideCeilSlope(Slope:FlxObject, Object:FlxObject, FacingEast:Bool):Void
	{
		// Set the object's touching flag
		...
		Object.y += 1; // Hack to prevent sticking to the ceiling and having this be called multiple times
	}
